...
router.post("/crearEscolta", validarJWT, async (req, res) => {
    const {idvecino, fecha, direccion, modalidad, detalle} = req.body;
    let terminar = false;
    ...
    if (terminar) { // No agregamos la alarma debido a que ya hay una sin terminar
        ...
        pool.query('INSERT INTO escolta (vecino_idvecino, fecha, estado, direccion, modalidad, detalle) VALUES ($1, $2, $3, $4, $5, $6)', [idvecino , fecha, 'activa', direccion, modalidad, detalle],async (err, rows) => {
            if (!err) {
                res.send({
                    code: 200,
...
router.get("/getEscoltas", validarJWT, async (req, res) => {
...
router.get("/getHistEscoltas", validarJWT, async (req, res) => {
...
router.post("/confirmarEscolta", validarJWT, async (req, res) => {
...
router.post("/terminarEscolta", validarJWT, async (req, res) => {
...
router.post("/cancelarEscolta", validarJWT, async (req, res) => {
    const { idescolta } = req.body;
